...
def init_db():
    ...
    #
    # 放在迁移之后，否则老库还没有 client_port 列。归属采样器每秒按 client_port
    # 更新一次，谁先建表谁就得把索引一起建上——这里是 addon，它比控制服务
    # 更早打开数据库。
    conn.execute("CREATE INDEX IF NOT EXISTS idx_client_port ON traffic(client_port)")
    conn.commit()
